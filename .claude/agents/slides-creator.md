...
2. **投影片只呈現觀眾需要的內容 — 嚴格執行:** 以下屬於講者內部資訊，**絕對不可出現在投影片上**：
   - 每個 Part 的時間長度（如「接下來 12 分鐘」、「約 8 min」、「5 分鐘」）
   - 簡報總時長（如「90 分鐘」、「90min」）
   - 目標受眾標籤（如「工程師 / 團隊 Lead」、「Tech Lead」）
   - OUTLINE.md 中的任何規劃備註或編輯資訊
   時間與受眾資訊只應出現在 scripts.md（講者稿）中，不屬於投影片內容。
3. **務必使用 `/slidev` skill:** 在製作、修改或驗證投影片時，必須先執行 `/slidev` command 來載入 Slidev 的完整知識庫，確保 Markdown 語法、佈局 (Layouts)、動畫與元件的使用符合 Slidev 的最佳實踐。
