...
<div class="text-base opacity-70 mb-6">依照可見範圍，MCP Server 分成三個層級——選對層級，才能讓工具在對的地方生效。</div>

<div class="grid grid-cols-3 gap-4 mt-4">

<div v-click class="p-4 rounded-lg border border-gray/30">
  <div class="text-2xl mb-2">🏠</div>
  <div class="font-bold text-base mb-1"><code>local</code></div>
  <div class="text-xs opacity-60 mb-3">僅本機使用者可見</div>
  <div class="text-xs opacity-80">個人工具、實驗性整合，不納入版控。設定存於 <code>~/.claude/settings.json</code>。</div>
</div>

<div v-click class="p-4 rounded-lg border border-gray/30">
  <div class="text-2xl mb-2">📁</div>
  <div class="font-bold text-base mb-1"><code>project</code></div>
  <div class="text-xs opacity-60 mb-3">整個專案團隊共享</div>
  <div class="text-xs opacity-80">標準化工具、Linter、CI 整合，隨 Repo 流通。設定存於 <code>.claude/settings.json</code>。</div>
</div>

<div v-click class="p-4 rounded-lg border border-gray/30">
  <div class="text-2xl mb-2">👤</div>
  <div class="font-bold text-base mb-1"><code>user</code></div>
  <div class="text-xs opacity-60 mb-3">跨機器、跨專案可用</div>
  <div class="text-xs opacity-80">個人偏好工具，所有工作環境都能使用。設定存於 <code>~/.claude.json</code>。</div>
</div>

</div>

<div v-click class="mt-6 p-4 bg-primary/10 border-l-4 border-primary rounded-r text-sm">
  <b>實務建議：</b> GitHub MCP 設為 <code>project</code> scope，讓整個團隊共享同一份設定；個人的 Monitoring MCP 則設為 <code>local</code>，不影響他人。
</div>

<!--
（顯示三個 Scope 分類）

MCP Server 依照可見範圍分成三個層級。

> [v-click 1] local scope

**local**：只有你自己在這台機器上看得到。適合實驗性工具，或是還沒確定要推廣的整合。設定寫在本機的 settings.json，不進版控。

> [v-click 2] project scope

**project**：整個 Repo 裡的人都能用。把 MCP 設定提交進 `.claude/settings.json`，新進成員 clone 後就能直接使用，不需要各自設定。

> [v-click 3] user scope

**user**：跨機器的個人設定。不管你在哪台電腦工作，這個 MCP 都跟著你。

> [v-click 4] 實務建議

實務上，把團隊標準工具放 project scope，個人習慣工具放 local 或 user。這樣新人上手就有完整工具鏈，而不需要口耳相傳每個工具怎麼裝。

> 過場：了解 Scope 之後，我們直接看 Demo。
-->
---
layout: center
class: text-center
